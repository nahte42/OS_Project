...
void VirtualMachine::run(int time_slice, fstream & in, fstream & out)
{
	...
	//***********BEGIN PAGING
	if((pc/8)  != ((pc-1)/8))// need to change and get new frame.
	{
		if(TLB.page_table[pc/8].valid){
			sr &= 0xfbff;
			
		}
		else{//page replacement algo
			int page_num = pc/8;
...
void VirtualMachine::load(fstream& object, int loc){
	
    object.seekg(std::ios::beg);
    int waste;
    int index = loc;
    for(int i = 0; i < pc; i++)//MAY NEED TO DO PC -1
	object >> waste;

    while(object>>mem[index]){
	index++;
	if(index%8 == 0)
		return;
    }
